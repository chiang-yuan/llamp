<script lang="ts">
	...
	import {
		DocsFeature,
		type DocsShellSettings,
	} from "$lib/layouts/DocsShell/types";
	...
	async function askLLaMP(question: string) {
		...
		if (response.ok) {
			...
			messages = [
				...messages,
				{ content: message, response: data.response },
			];
...
<div class="container h-full mx-auto flex justify-center items-center">
	<div class="space-y-10 text-center flex flex-col items-center">
		<h2 class="h2">LLaMP 🦙️🔮</h2>
		<h3 class="h3">Large Language model Made Powerful</h3>
		<!-- Animated Logo -->
		<!-- <figure>
		...
		<div class="chat-container">
			{#each messages as msg (index)}
				<div class="chat-bubble" class:from-agent={index % 2 === 0}>
					{msg.content}
				</div>
				<div class="clear" />
			{/each}
